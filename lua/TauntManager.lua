...
---Cooldown in seconds, after a taunt is played that blocks other taunts from playing.
---
---Defaults to 30 seconds.
---@field UnlockTimer integer
...
---@protected
function TM:__init(name, filename)
    self.Trash = TrashBag()
...
---@param fileName FileName
function TM:SetTauntFile(fileName)
...
---@return boolean
---@protected
function TM:TauntingCharacterCheck()
...
---@protected
function TM:TauntBasics(diagData, delaySecs)
    ...
    elseif type(diagData) == 'string' then
        ...
        error('TM: Invalid data for Taunt - must be table of strings or a string', 3)
    end
...
---@protected
function TM:PlayTaunt(tauntNum)
...
function TM:AddAttackTaunt(diagData, opAI, attacks)
...
function TM:AddAreaTaunt(diagData, area, category, brain, number)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param category EntityCategory
function TM:AddPlayerIntelCategoryTaunt(diagData, targetBrain, category)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param targetBrain AIBrain
---@param category EntityCategory
function TM:AddIntelCategoryTaunt(diagData, lookingBrain, targetBrain, category)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param diagData string|string[] Name of the dialogue or a list of names for the taunt. If it's a list, the taunt is picked randomly each time it's played. This name must be defined in the provided taunt file.
---@param unit Unit
function TM:AddPlayerIntelUnitTaunt(diagData, unit)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param unit Unit
---@param lookingBrain AIBrain
function TM:AddIntelUnitTaunt(diagData, unit, lookingBrain)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param diagData string|string[] Name of the dialogue or a list of names for the taunt. If it's a list, the taunt is picked randomly each time it's played. This name must be defined in the provided taunt file.
---@param unit Unit
function TM:AddUnitDestroyedTaunt(diagData, unit)
...
function TM:AddUnitKilledTaunt(diagData, unit, destroyed)
    local currNum, callback = self:TauntBasics(diagData)
...
function TM:AddUnitGroupDeathTaunt(diagData, unitTable, number)
    local currNum, callback = self:TauntBasics(diagData)
    if not number then
...
function TM:AddUnitGroupDeathPercentTaunt(diagData, unitTable, percent)
...
function TM:AddDamageTaunt(diagData, unit, percent)
    local currNum, callback = self:TauntBasics(diagData)
...
---@param category EntityCategory
---@param number integer
function TM:AddConstructionTaunt(diagData, brain, category, number)
    local currNum, callback = self:TauntBasics(diagData)
    number = number or 1
...
function TM:AddStartBuildTaunt(diagData, brain, category, delaySecs)
...
function TM:AddUnitsKilledTaunt(diagData, brain, category, number)
    local currNum, callback = self:TauntBasics(diagData)
    number = number or 1
...
---@param category EntityCategory
---@param number? integer Defaults to `1`.
function TM:AddEnemiesKilledTaunt(diagData, brain, category, number)
    local currNum, callback = self:TauntBasics(diagData)
    number = number or 1
...
function TM:AddTimerTaunt(diagData, seconds)
    local currNum, callback = self:TauntBasics(diagData)
...
function TM:AddCustomTaunt(diagData, customName)
    if self.CustomTaunts[customName] then
        WARN('TM: ' .. self.Name .. ' trying to add duplicate custom taunt: ' .. customName)
        return
    end
    local currNum, callback = self:TauntBasics(diagData)
...
---@param customName string
function TM:PlayCustomTaunt(customName)
    if not self.CustomTaunts[customName] then
        WARN('TM: ' .. self.Name .. ' custom taunt: ' .. customName .. ' not found')
        return
    end
    self:PlayTaunt(self.CustomTaunts[customName])
end

TauntManager = ClassSimple(TM)
...
function CreateTauntManager(name, fileName)
    ...
    if ScenarioInfo.TauntManagers[name] then
        error('TauntManager with name ' .. name .. ' already exists.', 2)
    end
